...
function createLinkProviderEmailHtml(email: string, providerLabel: string, url: string): string {
	const safeEmail = escapeHtml(email);
...
function createSetupPasswordEmailHtml(email: string, url: string): string {
	const safeEmail = escapeHtml(email);
	const safeURL = escapeHtml(url);
...
async function sendEmail(params: { html: string; subject: string; text: string; to: string }): Promise<boolean> {
