...
const AUTH_ERROR_MESSAGES: Record<string, string> = {
  ...
  invalid_code:
    "เซสชันการเข้าสู่ระบบหมดอายุหรือไม่ถูกต้อง กรุณาลองใหม่อีกครั้ง",
...
const AUTH_ERROR_PATTERNS: Array<[RegExp, string]> = [
  ...
  [/google login was cancelled/i, AUTH_ERROR_MESSAGES.google_login_cancelled],
  [/account not linked/i, AUTH_ERROR_MESSAGES.account_not_linked],
  [
    /different emails? not allowed|email(?:.*)doesn.?t match/i,
    AUTH_ERROR_MESSAGES.email_doesnt_match,
  ],
  [
    /state mismatch|request expired|invalid state/i,
    AUTH_ERROR_MESSAGES.state_mismatch,
  ],
  [
    /invalid (email|password|credential)|wrong password/i,
    AUTH_ERROR_MESSAGES.invalid_credentials,
  ],
  [
    /user already exists|another email/i,
    AUTH_ERROR_MESSAGES.user_already_exists_use_another_email,
  ],
  [/username.*taken|username.*exists/i, AUTH_ERROR_MESSAGES.username_is_taken],
  [
    /failed to create user|unable to create user/i,
    AUTH_ERROR_MESSAGES.failed_to_create_user,
  ],
  [
    /failed to get user info|unable to get user info/i,
    AUTH_ERROR_MESSAGES.unable_to_get_user_info,
  ],
];
...
export function getThaiAuthErrorMessage(
  error: unknown,
  fallback: string,
): string {
