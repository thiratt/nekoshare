...
export function AppI18nProvider({ children, initialLanguage }: { children: ReactNode; initialLanguage?: string }) {
	const [language, setLanguageState] = useState<AppLanguage>(() => resolveInitialLanguage(initialLanguage));
	const [syncLanguageFromAccount, setSyncLanguageFromAccountState] = useState<boolean>(() =>
		getStoredSyncPreference(),
	);
