...
import { createTimeoutError, createWebSocketError, ErrorCategory, ErrorSource, NkWebSocketError } from "./errors";
import { getPacketTypeName } from "./protocol";
...
interface PendingRequest {
	...
	readonly reject: (err: NkWebSocketError) => void;
...
export class NekoSocket {
	...
	public setAutoReconnect(enabled: boolean): void {
		...
		if (!enabled && this.status === SocketStatus.RECONNECTING) {
			this.updateStatus(SocketStatus.DISCONNECTED);
		}
	}
	...
	public request(
		...
		return new Promise((resolve, reject) => {
			...
			const wrappedReject = (error: NkWebSocketError) => {
				clearTimeout(timer);
				reject(error);
			};
			...
			try {
				...
				this.pendingRequests.delete(requestId);
				reject(
					error instanceof NkWebSocketError
						? error
						: createWebSocketError(
