...
export type ShareDetailConnectionState =
  | "connecting"
  | "handshaking"
  | "recovering"
  | "disconnected";
export type ShareDetailState =
  | TransferDeliveryState
  | ShareDetailConnectionState
  | "sending";
...
function normalizeDeliveryState(
  state?: ShareDetailState | ShareDetailFileStatus | null,
): TransferDeliveryState {
  switch (state) {
    case "sending":
      return "transferring";
...
function getDeliveryBadgeConfig(
  state: ShareDetailState | ShareDetailFileStatus,
  direction?: TransferDirection,
) {
...
function getAvailableActions({
  ...
  const normalized = normalizeDeliveryState(state);
  const canPause =
    normalized === "transferring" && resolveCapability(true, actions?.canPause);
  const canResume =
    normalized === "paused" && resolveCapability(true, actions?.canResume);
  const canRetry =
...
function getAggregateDeliveryState(
  items: Array<{ state?: ShareDetailState; status?: ShareDetailFileStatus }>,
) {
  if (items.length === 0) return "queued" satisfies TransferDeliveryState;

  const states = items.map((item) =>
    normalizeDeliveryState(item.state ?? item.status),
  );
...
function getRecipientDisplayState(
  recipient: ShareDetailRecipient,
): TransferDeliveryState {
  if (recipient.files.length > 0)
    return getAggregateDeliveryState(recipient.files);
...
function getRecipientStats(recipient: ShareDetailRecipient) {
  ...
  const transferredBytes = recipient.files.reduce(
    (sum, file) =>
      sum + Math.max(0, Math.min(file.transferredBytes, file.size)),
    0,
  );
  const failedFiles = recipient.files.filter(
    (file) => normalizeDeliveryState(file.status) === "failed",
  ).length;
  const completedFiles = recipient.files.filter(
    (file) => normalizeDeliveryState(file.status) === "completed",
  ).length;
...
function formatRouteLabel(
  ...
  if (route) {
    ...
    if (route.type === "unknown" && route.protocol === "unknown")
      return fallback ?? "—";
...
function StatusBadge({
  ...
  return (
    <Badge
      variant="outline"
      className={cn("gap-1.5 rounded-full", config.badgeClassName)}
    >
...
function EmptyDetail({
  linkComponent,
  backHref,
}: IncludeLinkComponentProps & { backHref: string }) {
  return (
...
export function ShareDetailUI({
  ...
  const sessionState =
    recipientStates.length > 0
      ? getAggregateDeliveryState(recipientStates)
      : normalizeDeliveryState(data.state);
  const sessionActions = getAvailableActions({
    state: sessionState,
    actions: data.actions,
    scope: "session",
  });
  const recipientStats = data.recipients.map((recipient) =>
    getRecipientStats(recipient),
  );
  const totalFiles = data.recipients.reduce(
    (sum, recipient) => sum + recipient.files.length,
    0,
  );
  const failedFiles = recipientStats.reduce(
    (sum, stats) => sum + stats.failedFiles,
    0,
  );
  const completedFiles = recipientStats.reduce(
    (sum, stats) => sum + stats.completedFiles,
    0,
  );
  const sessionTransferredBytes = recipientStats.reduce(
    (sum, stats) => sum + stats.transferredBytes,
    0,
  );
  const sessionTotalBytes = recipientStats.reduce(
    (sum, stats) => sum + stats.totalBytes,
    0,
  );
  ...
  const sessionLabel =
    ...
    data.recipients.some(
      (recipient) => getRecipientDisplayState(recipient) === "completed",
    )
      ? "หยุดบางส่วน"
      : getDeliveryLabel(sessionState, data.direction);
  const sessionPrimaryAction = getPrimaryAction(sessionActions);
  const selectedRecipientStats = selectedRecipient
    ? getRecipientStats(selectedRecipient)
    : null;
  ...
  const selectedRecipientActions = selectedRecipient
    ...
    : null;
  const selectedRecipientPrimaryAction = selectedRecipientActions
    ? getPrimaryAction(selectedRecipientActions)
    : null;
  const hasMultipleRecipients = data.recipients.length > 1;
  const showFileActions = (selectedRecipient?.files.length ?? 0) > 1;
  const selectedRouteLabel = formatRouteLabel(
    selectedRecipient?.route,
    data.pathLabel,
  );
  ...
  return (
    <CardTransition
      className="flex h-full flex-col gap-0 overflow-hidden pb-0"
      tag="share-detail-card"
    >
      <CardHeader>
        <div className="flex flex-wrap items-start gap-3">
          ...
          <div className="ms-auto flex max-w-full flex-wrap items-center justify-end gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => onCopyId?.(data.id)}
            >
              <LuCopy />
              คัดลอก ID
            </Button>
            {sessionActions.canOpenFolder ? (
              <ActionButton action="openFolder" onClick={onOpenFolder} />
            ) : null}
          ...
          {hasMultipleRecipients && (
            <section className="flex min-h-0 flex-col overflow-hidden rounded-lg border bg-card">
              <div className="border-b px-4 py-3">
                ...
                <p className="text-sm text-muted-foreground">
                  เลือกผู้รับเพื่อดูไฟล์และจัดการรายคน
                </p>
              </div>
              <ScrollArea className="min-h-0 flex-1">
                <div className="space-y-2 p-3">
                  {data.recipients.map((recipient) => {
                    ...
                    const badgeLabel = recipientStats.hasPartialFailure
                      ? "บางส่วน"
                      : undefined;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
                          <span>
                            {recipientStats.completedFiles}/
                            {recipient.files.length} ไฟล์
            ...
            <div className="flex items-center justify-between gap-3 border-b px-4 py-3">
              <div className="min-w-0">
                <h2 className="truncate font-medium">
                  {selectedRecipient?.name ?? "ไม่มีผู้รับ"}
                </h2>
              ...
              {selectedRecipientStats ? (
                <StatusBadge
                  state={selectedRecipientStats.state}
                  direction={data.direction}
                />
              ) : null}
            </div>
                  ...
                  fileState === "completed"
                    ...
                    : file.size > 0
                      ? clampPercent(
                          (Math.max(
                            0,
                            Math.min(file.transferredBytes, file.size),
                          ) /
                            file.size) *
                            100,
                        )
          ...
          <aside className="min-h-0 space-y-4 overflow-hidden">
            <div className="rounded-lg border bg-card p-4">
              ...
              <div className="mt-4 space-y-3 text-sm">
                ...
                <DetailRow
                  label="เริ่มเมื่อ"
                  value={formatOptionalDate(data.startedAt)}
                />
                <DetailRow
                  label="อัปเดตล่าสุด"
                  value={formatOptionalDate(data.updatedAt)}
                />
                <div className="flex justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {sessionLabel} · {data.recipients.length} รายการปลายทาง ·{" "}
                  {completedFiles}/{totalFiles} งานไฟล์เสร็จแล้ว
                  {hasPartialFailure
                    ? ` · มีบางรายการล้มเหลว (${failedFiles})`
                    : ""}
                </p>
              </div>
            </div>
